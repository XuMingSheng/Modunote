...
#[utoipa::path(
    ...
    tag = "block_links",
    responses(
        (status = StatusCode::NO_CONTENT, description = "Parent link deleted successfully"),
...
pub async fn delete_block_parent_link(
    ...
    state
        ...
        .delete_by_block_ids(parent_id, id)
        .await
        .map_err(|e| match e {
            BlockDirectionalLinkRepositoryError::NotFoundByBlocks { .. } => HandlerError::NotFound,
            _ => {
                error!("Failed to delete block directional link {parent_id} -> {id}: {e}");
                HandlerError::Anyhow
            }
        })?;
