...
use super::{request::CreateBlockChildLinkRequest, response::CreateBlockChildLinkResponse};
use crate::AppState;
...
#[utoipa::path(
    ...
    tag = "block_links",
    responses(
        (status = StatusCode::CREATED, description = "Block link created successfully", body = CreateBlockChildLinkResponse),
...
pub async fn create_block_child_link(
...
) -> Result<(StatusCode, Json<CreateBlockChildLinkResponse>), HandlerError> {
    ...
    let link = state
        ...
        .map_err(|e| match e {
            ...
            _ => {
                error!("Failed to create block directional link {id} -> {block_to_id}: {e}");
                HandlerError::Anyhow
            }
        })?;
